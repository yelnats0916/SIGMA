document.addEventListener('DOMContentLoaded', () => {
    ...
    // Close sidebar on link click (Mobile) — exclude nav-courses (submenu toggle)
    navLinks.forEach(link => {
        link.addEventListener('click', () => {
            if (window.innerWidth < 1024 && link.id !== 'nav-courses') {
                const sidebar = document.getElementById('sidebar');
                const overlay = document.getElementById('sidebar-overlay');
    ...
    function initSubjectParentSidebar() {
        ...
        const openSubjectTopic = (programKey, subjectId, label) => {
            ...
            // On mobile: close the sidebar after navigating to a subject
            if (window.innerWidth < 1024) {
        ...
        parent.addEventListener('click', event => {
            ...
            if (window.innerWidth >= 1024) switchTab('nav-courses');
        });
        document.querySelectorAll('#subjects-submenu .nav-sublink').forEach(link => {
    ...
    function _renderTopicContentMain(subjectId, topicIdx, tab) {
        ...
        const innerContent = mainContent.replace(tabNavHtml, '');

        page.innerHTML = `
            <div class="mx-auto max-w-[1024px] w-full px-4 py-0 min-w-0">
                ${tabNavHtml}
                <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm min-h-screen overflow-hidden">
    ...
    function syncStudentSectionsNavState(showChildren = Boolean(activeStudentClassroomId)) {
        ...
        const shouldShowChildren = showChildren && isStudentClassroomDetailVisible();
        ...
        parent.classList.toggle('active', shouldShowChildren);
        parent.classList.toggle('open', shouldShowChildren && !isCollapsed);
        parent.querySelector('.sidebar-group-chevron')?.classList.toggle('rotate-90', shouldShowChildren && !isCollapsed);
    ...
    function renderStudentClassroomSectionsSidebar(activeClassroomId = activeStudentClassroomId, showCollapsedOverlay = true) {
        ...
        content.querySelectorAll('[data-classroom-id]').forEach(button => {
            button.addEventListener('click', () => showClassroomDetail(button.dataset.classroomId));
        });
    ...
    document.getElementById('nav-classrooms')?.addEventListener('click', event => {
        if (!isStudentClassroomDetailVisible()) return;
        event.preventDefault();
        event.stopImmediatePropagation();
        const isCollapsed = document.body.classList.contains('sidebar-collapsed') && window.innerWidth >= 1024;
        renderStudentClassroomSectionsSidebar(activeStudentClassroomId, isCollapsed);
    ...
    function showClassroomDetail(classroomId, pushState = true) {
        ...
        renderStudentClassroomSectionsSidebar(classroomId);
